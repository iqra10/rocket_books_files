...
class Rocket_Books {
	...
	private function load_dependencies() {
		...
		/**
		 * The class holding methods that are shared across all the classes
		 */
		...
		*/
	    ...
	    /**
		 * The class responsible for defining all actions for registering our custom post types
		 */
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-rocket-books-post_types.php';
	...
	public function get_version() {
     ...
     /**
	 * Defining all action and filter hooks for registering our custum post types
	 */
    public function define_post_type_hooks() {
		...
		// $this->loader->add_action( 'do_meta_boxes_book', $plugin_post_types ,'register_metabox_book' , 10 , 3 );

		// $this->loader->add_action( 'save_post', $plugin_post_types ,'metabox_save_book' , 10 , 3 );
//		$this->loader->add_action( 'save_post', $plugin_post_types ,'register_metabox_book' , 10 , 3 );
    ...
    public function define_shortcode_hooks() {
		...
		$plugin_shortcodes = new Rocket_Books_Shortcodes( $this->get_plugin_name(), $this->get_version() );
